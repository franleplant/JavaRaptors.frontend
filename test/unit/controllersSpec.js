...
describe('Controllers', function(){
	...
	describe('CreateController',  function () {
		...
		beforeEach(inject(function(_$httpBackend_, $rootScope, $controller, $location) {
			...
			location = $location;
			scope = $rootScope.$new();
			ctrl = $controller('CreateController', {$scope: scope});
		}));
		...
		describe('rm_author method', function () {
			it('should remove the author in the position 2 of the authors array', function() {
				...
				scope.new_book.authors = ['author0', 'author1', 'author2', 'author3'];

				scope.rm_author(2);

				expect(scope.new_book.authors.length).toBe(  3  );
				expect(scope.new_book.authors[2]).toBe(  'author3'  );
			});
		});
		...
		describe('add_author method', function () {
			it('should add an author to the authors array', function() {
				...
				expect(scope.new_book.authors.length).toBe(  1  );

				scope.add_author();

				expect(scope.new_book.authors.length).toBe(  2  );
			});
		});
		...
		describe('rm_copy method', function () {
			it('should remove the copy in the position 0 of the copys array', function() {
				...
				expect(scope.new_book.copys.length).toBe(  2  );

				scope.rm_copy(0);

				expect(scope.new_book.copys.length).toBe(  1  );
				expect(scope.new_book.copys[0].comments).toBe(  'new copy2'  );
			});
		});
